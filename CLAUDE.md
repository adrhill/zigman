...
ziglang.org/documentation/master/ (fetchurl)
  → pandoc -f html -t typst --lua-filter=filter.lua --template=template.typ
  → sed (strip toc- links)
  → content.typ
...
- **`template.typ`** — Minimal Pandoc Typst template (just `$body$`). Document styling is handled in `main.typ`.
- **`main.typ`** — Typst entry point. Sets 6"×9" page size, margins, fonts, heading/code/table/link styles, title page, and table of contents. Includes `content.typ` (generated by Pandoc).
...
2. **Convert**: Pandoc converts HTML → Typst markup, using `filter.lua` for cleanup and `template.typ` as the output template. A `sed` post-process strips Pandoc's `#link(<toc-...>)` wrappers from headings.
3. **Typeset**: Typst compiles `main.typ` (which includes `content.typ`) into a 6"×9" PDF.
...
- **Pandoc** — converts the Zig reference HTML to Typst markup. Uses `--lua-filter` and `--template` flags.
